...
const fetchDestinationImage = async (destination) => {
  ...
  try {
    const response = await fetch(
      `https://api.unsplash.com/search/photos?query=${encodeURIComponent(destination)}&client_id=${apiKey}&per_page=1`
    );
  ...
  return (
    <>
      ...
      <div className="flex justify-between items-center">
        ...
        <Link
          to={`https://www.google.com/maps/search/?api=1&query=${destination}`}
          target="_blank"
        >
